...
Save schema is now **4**. The browser key remains `system-sandbox:first-level:v2` deliberately so existing boards are found without moving or deleting user data. Schemas 2 and 3 migrate with an empty snapshot shelf; existing boards and surviving certificate records are preserved. Older builds reject schema 4 instead of silently dropping its shelf.

The save contains three separate collections:
...
- `snapshots`: at most eight uniquely named detached design copies (id, name, nodes, edges). No frames or private mapping experiment data. Names have 1–48 characters and are not graded; malformed or duplicate shelf entries cause validation to fail rather than silently deleting experiments.
...
Schema-2 migration accepts valid original `p99`-shaped summaries and renames that field. Unversioned legacy reports are bound to **frozen identifiers for the original simulator and scenarios**, never to whatever the current constants happen to be in a future release. Explicit older versions are preserved. Schemas 3 and 4 do not silently reconstruct missing certificates from history or assume that unversioned evidence is current.

Migration can preserve only evidence that still exists. Passes already evicted by an older build cannot be recovered from no record. Versioned portable JSON backups preserve the board/settings/certificates/shelf, exclude session experiment data and traffic frames, and accept raw schema-2/3/4 saves. Imports are size-limited, validated, previewed and explicitly confirmed. Restoring or resolving a conflict selects the whole save including its shelf; there is no automatic shelf merge.

Snapshot writes use the same serialized save session as normal edits, and local UI state changes only after a safe write succeeds. A load opens a copy, never a mutable reference to the snapshot. Before loading, the current design (including geometry) is added to the shelf unless an identical copy exists. If there is no room, loading is blocked without evicting anything. Loading clears current frames, undo history and the mapping experiment; saved certificates still match the restored behavior. Saving/removing a snapshot does not clear the active recording. Removal requires confirmation and the prior save is retained in the rolling recovery slot. Download backups for longer-term recovery; this is a bounded snapshot shelf, not a version-control merge system.

Participating tabs coordinate compare-and-write using the browser Web Locks API. A stale tab pauses autosave and retains its local board; explicit conflict resolution can load the saved copy or replace it after saving a recovery checkpoint. A source changed since review aborts the replacement. Changed autosaves checkpoint the previous valid save, while no-op writes do not discard recovery. Unknown/corrupt primary data is not automatically overwritten; explicit replacement first preserves its original bytes under `system-sandbox:first-level:protected-original`. Recovery and protected-original slots each retain only their latest copy; download files for copies independent of browser storage.
...
The design shelf can compare two snapshots or the current working board. Every side is validated against the same current functional rules and budget before traffic measurement. Both designs run all three canonical scenarios independently from cold state using `runChapter`; no saved report, cache state or certificate is consumed. Results record current model/scenario identifiers and contain summary metrics only, not archived frames. This aggregate model is deterministic and has no random seed. A comparison does not award passes, unlock challenges or mutate the working board, recording, shelf or persisted save. It remains available without storage writes.

Tables show cost, each challenge's worst-sample rejection percentage and peak estimated latency, with B minus A deltas (rejection differences in percentage points). Display rounding never changes the underlying pass checks. An unavailable latency stays unavailable, not zero. Lower numbers are not a universal ranking: completed-request latency must be considered alongside rejected work and costs. Structural/budget failures are explained without inventing measured traffic. Configuration changes are matched by saved node identity, ignoring geometry and edge IDs; rebuilding nodes counts as removal/addition. Changing a selection clears previous comparison results. Results are session-only and do not establish real-world performance or learning.

## Controlled workload experiment

The forecast's **Explore traffic mix** panel compares three profiles on a normalized copy of the current design: 80% reads / 92% hot share, 80% reads / 15% hot share, and 15% reads / 15% hot share. Hot share is the portion of redirect reads revisiting the popular link, not a guaranteed cache hit rate or a count of unique visitors. Examples per 100 requests introduce the percentages.

Every side uses the same selected peak (100, 500, 1000, 1400 or 2400 requests/sec), five-second ramp and 12-second duration, with independent cold state and the unchanged `runChapter` simulator. First-versus-second isolates reuse; second-versus-third isolates read/create mix. In an uncached design, changing reuse alone has no model effect. API/cache/storage capacity can still cause upstream rejection, so low downstream offered work is not automatically a saving.

The table shows final-sample reads, writes and rejections per component, final cache hit share and whole-run worst rejection. Database write work includes weighted allocation; memory-cache writes are optional fills; edge-cache writes include creation forwarding and fills. Extra API allocation compute affects utilization, not extra incoming requests. Incomplete graphs produce structural explanations without simulated measurements. Structurally valid over-budget designs may be explored with their cost warning: the lab is ungraded, not a route around the challenge budget.

No certificate/report is exposed for awarding progress. The lab never changes the board, active challenge, saved data, undo history or captured playback frames. It is available while building/reviewing or paused, not during live playback. Input changes, behavior changes and reopening clear the comparison. This is a controlled experiment within Level 1, not a new playable level or free-form production load test.
...
The HUD distinguishes live, paused, completed review, ungraded partial review and building. End run first pauses for explicit confirmation; cancellation does not resume automatically. Confirming retains captured frames for inspection but never creates a report or certificate for the incomplete stage. Earlier completed stages retain their passes. Paused scrubbing is read-only; stepping/resuming returns to the latest sample. Recordings remain session-only and are cleared by a behavior change or another run; geometry changes preserve them.
...
176 unit/static-render/orchestration tests pass as of 7 September 2026; the production build succeeds. The representative balance grid is also reproduced against its versioned snapshot. `npm run verify -- --runtime /path/to/node_modules` runs all tests/build and 23 isolated browser scripts against a private production preview; `--suite smoke` is a labelled smaller subset. Checks cover campaign/editor flows, recorded outcomes, mapping experiments, edit history, connection previews, contextual help, portable backups, two-tab/blocked-storage behavior, component contracts/choices, cloud-link navigation, workload experiments, pass recaps, capacity previews, structural review, text navigation, motion, controlled playback, explicit run modes, named snapshots, fair comparisons and selected solid-surface text readability. These checks do not establish novice learning, full assistive-technology access or comprehensive visual QA. The broader backlog readiness gate remains open.
