...
**Quick dialog-access slice · 7 September 2026:** L1-043/046: component-choice and workload dialogs now retain a visible 44px Back button while scrolling. New hit-testing exposed a pre-existing fixed-dialog width issue: document overflow checks passed even when a dialog extended outside a narrow viewport. Both dialogs now have an explicit viewport width cap. Browser checks at 768/390/320px assert actual panel bounds, button visibility/hit targets at the bottom, successful click-to-close and retained player state. Verification: 176 unit tests, production build and the two affected browser checks pass; the full suite was not rerun for this scoped change.

**Workload-shape slice · 7 September 2026:** After pushing the FB-001–003 priority batch (`04242fd`), L1-019 now adds concrete open/create counts to the forecast and an optional **Explore traffic mix** experiment. Three matched-volume profiles isolate repeated versus scattered reads, then opens versus creations, on the unchanged player board. All share a five-second ramp, 12-second duration and selected peak; each starts cold. The table distinguishes final-sample per-component offered work from worst rejection over the whole run. Results never certify a design, change the selected challenge, mutate a save or replace the paused recording; invalid boards receive structural reasons instead of manufactured traffic. Input changes/reopening clear results. Six unit tests cover canonical equality, independent cold starts, no imaginary uncached savings, durable writes, purity, invalid inputs and unobserved components. Targeted browser checks cover input invalidation, saved/recorded-state preservation, keyboard table navigation and compact layouts. Verification: 176 tests, production build, all 12 browser smoke checks and the separate editor check pass. Novice comprehension and optional-contract design (L1-020/021) remain open; the full 23-script release suite has not been rerun for this second slice.

**Essential-text readability slice · 7 September 2026:** L1-041/046 are partially addressed with stronger opaque text and larger critical HUD/component/inspector/trace/result labels while preserving the clay layout. A [bounded audit](level-1-readability-audit.md) checks computed solid-surface contrast and font sizes across build/trace/pass/fail and 768/390/320px layouts, plus reachable retry controls. Initial post-change sampling: 371 text/style observations, minimum 4.747:1 contrast and 10px type. Prior nominal CSS contrasts included 2.714:1 footer labels and 3.566:1 component tiers. Canvas/gradient/disabled-state coverage, real text/browser zoom and human access/readability testing remain open; no full conformance claim is made.

**Fair comparison slice · 7 September 2026:** L1-034 now compares two named snapshots or the working board using fresh canonical runs of all three challenges, each independently cold. No historical reports, cache warmth or certificate results enter the calculation. The panel shows cost, worst-sample rejection, peak estimated latency and signed B−A differences alongside component/tier/allocation/call changes. Invalid/over-budget setups are blocked with explanations. Selection changes clear stale results; comparisons neither load a board nor alter recordings, snapshots, history, unlocks or certificates. No random seed is needed in this deterministic aggregate model. Unit/browser checks cover canonical equality, alternate passing trade-offs, identical-design controls, invalid inputs, retained state, focus and compact tables. This supplies a comparison tool, not proof that learners can justify trade-offs.

**Named snapshot slice · 6 September 2026:** L1-033 now has a board-toolbar design shelf with eight named immutable snapshots, cost/current-rule pass counts, confirmed loading/removal and automatic checkpointing of the current board before loading. Full shelves block replacement instead of evicting a design. Schema 4 migrates schema-2/3 boards and surviving passes; portable backups and cross-tab conflict choices include the shelf. Local state changes only after safe storage succeeds. Saving retains evidence; loading resets session recordings/undo/mapping experiments. Unit/browser coverage includes migration, malformed shelves, bounded retention, duplicate names, copy independence, reload, backup round-trips, conflicts and denied writes. This is a snapshot workflow, not automatic merging or a learning-efficacy claim; fair A/B testing remains L1-034.

**Explicit run modes slice · 6 September 2026:** L1-030 now labels build, live, paused, completed recording and ungraded partial recording with contextual edit permissions. End run pauses and confirms; cancellation stays paused, confirmation preserves captured samples, and no report/certificate is created for an unfinished challenge. Paused scrubbing does not advance the run; restart starts cold. Focus returns to a usable control, and already-earned suite passes remain intact. Layout edits keep evidence; behavioral edits clear it. Model and browser regressions exercise these transitions; named branches remain L1-033.

**One-command verification slice · 6 September 2026:** L1-052 now has `npm run verify -- --runtime /path/to/node_modules`: model/component/orchestration tests, production build and all 17 current browser scripts using a runner-owned production preview. It fails fast with a nonzero exit, guards occupied ports, isolates screenshots/profile state, and cleans up its own preview; a named smoke subset is available but not represented as full coverage. Older campaign/editor scripts now close their browser on failures and accept an isolated screenshot directory. Branch restore checks remain dependent on L1-033; real assistive-technology and learner studies remain open.

**Representative balance audit · 6 September 2026:** L1-017 enumeration is implemented in `scripts/balance-audit.mjs` with a [versioned report and exact snapshot](level-1-balance-audit.md). Of 2,610 candidates, 630 are budget-eligible and 144 pass all three contracts. All seven architecture families have passing examples; direct origin, API cache and edge cache have nondominated examples. Dedicated allocation is valid but has no frontier advantage in this tested grid. No requirements or simulator rules were changed to manufacture component usefulness. The audit covers finite templates, not every graph or a learner study. Its snapshot and alternate fixtures have automated regression coverage; follow-up balance experiments remain open.

**Controlled playback slice · 6 September 2026:** L1-036 now provides 1×/2×/4× wall-clock playback and one fixed 0.2-second sample per paused Step. The extracted controller reproduces every canonical frame/report exactly; suite stages start cold, failed stages stop the suite, and manual boundary completion preserves pause/focus behavior. Hidden-page events pause rather than skip/catch up samples; visible return requires explicit Resume. **136 tests, isolated stepping/speed/suite/visibility-fixture browser checks and the production build pass.** Browser lifecycle behavior beyond the exercised fixture and individual-event simulation are not claimed.

**Traffic motion slice · 6 September 2026:** L1-045 now separates moving particles from simulation progress. The player can turn traffic-path animation off without stopping samples or changing outcomes; the local preference survives reload when storage is available. A live reduced-motion subscription immediately overrides particles, including illustrative traces with no ongoing simulation ticks. Text navigation and telemetry remain available. **131 tests, pixel-based animation/freeze browser checks, unchanged-result comparisons and the production build pass.** This addresses traffic motion; it does not certify all sensory/accessibility requirements or introduce audio.

**Text system navigator slice · 6 September 2026:** L1-044/L1-042 are advanced, not declared complete: an optional non-modal system list provides semantic component/call lists, uniquely named replicas, node/call inspection and service-call planning without selecting a canvas port. Stable paused/stopped sample counts expose admissions/rejections in text. Existing inspector controls supply configuration, movement and removal; keyboard checks build a replicated graph, inspect/disconnect/undo, and preserve focus. Board/port/inspector labels agree on replica names. **128 tests, navigator and two-tab regression browser checks, plus the production build pass.** Actual assistive-technology sessions, full contrast/target audits and all equivalent graph operations remain part of the readiness gate.

**Structural review slice · 6 September 2026:** L1-031 now has an expandable list of every distinct blocking connection issue and a component-local explanation. Inspect actions focus the actual node and reveal its warning without modifying the board or recording. Warnings update immediately after repairs; dedicated allocation errors keep the sequence/random alternatives available. Structural validity is explicitly separate from tested capacity. **125 tests, isolated keyboard/focus/repair/compact-width browser checks and the production build pass.**

**Capacity decision slice · 6 September 2026:** L1-028 now previews tier changes with current/proposed capacity, component price delta, whole-board budget and over-budget consequences before explicit Apply. Preview/cancel leaves the save, recording and history untouched; applying is one reversible behavioral edit. Storage read/write ceilings are explained as a shared budget, and cache coverage is not presented as a promised hit rate. **122 tests, isolated capacity browser checks, outcome/editor regressions and the production build pass.** The component teaching slice supplies progressive responsibility details; this does not claim a novice usability study.
