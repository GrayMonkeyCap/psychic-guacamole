...
157 unit/static-render/orchestration tests pass as of 6 September 2026; the production build succeeds. The representative balance grid is also reproduced against its versioned snapshot. `npm run verify -- --runtime /path/to/node_modules` runs all tests/build and 19 isolated browser scripts against a private production preview; `--suite smoke` is a labelled smaller subset. Checks cover campaign/editor flows, recorded outcomes, mapping experiments, edit history, connection previews, contextual help, portable backups, two-tab/blocked-storage behavior, component contracts, pass recaps, capacity previews, structural review, text navigation, motion, controlled playback, explicit run modes and named snapshots. These checks do not establish novice learning, full assistive-technology access or comprehensive visual QA. The broader backlog readiness gate remains open.
