...
Omit `--runtime` if Playwright is installed in this repository, or set `PLAYWRIGHT_NODE_MODULES`. The runner checks dependencies, runs unit/component tests, builds production assets and exercises all 23 browser scripts against its own localhost preview on port 5181. It stops on the first failure and exits nonzero; it never downloads dependencies or stops an existing server. Use `--port 5182` if occupied. Its preview is closed on success, failure or interruption; the player's running development preview and browser profile are not used. Screenshots go to a unique ignored `.test-artifacts/verify-*` directory.

`--suite smoke` explicitly selects twelve browser checks (campaign, connections, backups, concurrent saves, text navigator, playback, run modes, design shelf, comparison, readability, component choices and workload experiments), plus all unit tests/build; it is not the full release check. `--help` lists options. Allow several minutes for the full suite. Automated checks do not establish accessibility conformance or learning outcomes.
...
Earned passes are stored independently of the 12 most recent attempts, keyed by behavioral design fingerprint and model/scenario versions. Save schema 4 migrates existing schema-2/3 records in place under the same browser-storage key, preserving the board and surviving passes and adding an empty design shelf. Earlier-rule passes remain recorded but do not certify the current rules. `src/levelProgress.test.js` covers retention, migration and threshold boundaries; `src/progressViews.test.jsx` checks that both screens present the same certification state.
...
**Help me choose** previews all six components before placement, with current-board reasons, alternatives and clearly labeled forecast versus recorded-sample evidence. The inspector keeps that context visible. Optional **Go deeper** links connect the concepts to official AWS/GCP settings (and PostgreSQL allocation/uniqueness semantics); no game tier is a real cloud SKU or quote. The [feedback implementation notes](docs/player-feedback-2026-09-07.md) document scope and sources. Campaign verification now exercises actual wheel and keyboard scrolling, not just anchor navigation.

**Explore traffic mix** is an optional ungraded experiment beside the forecast. It compares repeated reads, scattered reads and new-link creation using the same board, peak rate, ramp and duration, independently cold. Results show per-component work at the final sample and worst rejection over each whole run. It never changes a challenge, recording, save or pass; a larger traffic number is not a new level. See `src/workloadLab.js` and `scripts/verify-workload-lab.mjs` for the controlled comparison and regressions.
...
The HUD distinguishes **Build / Live / Paused / Test recording / Partial recording** and explains allowed actions. Paused traffic can be scrubbed without changing the run. **End run** pauses first and asks for confirmation; cancel leaves it paused. Confirming keeps captured samples for inspection without grading the unfinished challenge. Layout changes preserve evidence; behavioral changes clear it. `scripts/verify-run-modes.mjs` checks transition safety, focus, suite passes and compact layouts.

**Design shelf** keeps up to eight named, immutable snapshots. Load a working copy with explicit confirmation; the current board is checkpointed first unless an identical snapshot already exists. A full shelf blocks replacement instead of evicting a design. Saving a snapshot preserves the current recording; loading clears session evidence/undo/mapping experiments but keeps earned passes. Schema-4 backups include the shelf; removal is confirmed and the previous save is kept in recovery. Safe storage and conflict resolution are required for shelf writes. `scripts/verify-design-shelf.mjs` tests migration, snapshots, cancellation, backup round-trips, reload, current-rule passes, two-tab conflicts and denied writes.

Inside the shelf, **Compare designs** tests any two snapshots or the working board under identical cold-start traffic. It shows all three contracts, cost/rejection/estimated-latency deltas and component/configuration/call changes. Invalid or over-budget setups are explained instead of presented as traffic measurements. Fresh comparison runs never reuse historical scores, change the board/recording, unlock challenges or award passes. `scripts/verify-design-comparison.mjs` checks exact results, equal-design controls, stale-result clearing, read-only behavior, focus and compact tables.

Important HUD, component and result text now uses stronger contrast and larger type. `scripts/verify-readability.mjs` checks selected solid-background text in build/trace/pass/fail states and compact layouts. The [readability audit](docs/level-1-readability-audit.md) documents measurements and limits; it does not claim full accessibility conformance, canvas coverage or real browser-zoom qualification.
