      ...
      <aside className="l1-inspector l1-paper">
        ...
        {trace ? <>
          ...
          return <><div className="l1-inspector-heading"><span className="l1-inspector-icon" style={{ background: config.color }}><Icon size={24} /></span><div><h2>{config.name}</h2><small>{config.verb}</small></div></div><p>{config.role}</p>
            ...
            {load && <div className="l1-node-telemetry"><span>Read operations <b>{round(load.reads)}/s</b></span><span>Write operations <b>{round(load.writes)}/s</b></span><span>Waiting <b>{round(load.queue)}</b></span>{['cache', 'cdn'].includes(node.type) && <span>Served from memory <b>{round(load.hits)}/s</b></span>}</div>}
